...
void ShaderUnit::initializeInputRegisters(uint32_t threadCount) {
    ...
    Isa::RegisterIndex inputRegisterIndices[4] = {};
    ...
    if (isFs) {
        ...
        for (int vertexIndex = 0; vertexIndex < verticesInPrimitive; vertexIndex++) {
            for (int inputIndex = 0; inputIndex < inputsCount; inputIndex++) {
                ...
                Isa::RegisterIndex registerIndex = registerAllocator.allocate();
...
void ShaderUnit::appendOutputRegistersValues(uint32_t threadCount, uint32_t *outputStream, uint32_t &outputStreamSize) {
    ...
    Isa::RegisterIndex registerIndices[4] = {};
...
void ShaderUnit::zeroInitializeUnusedRegisters(uint32_t threadCount) {
    ...
    const size_t uniformsCount = isaMetadata.uniformsCount;
    for (size_t uniformIndex = 0; uniformIndex < uniformsCount; uniformIndex++) {
        const Isa::RegisterIndex registerIndex = getUniformRegisterIndex(uniformIndex);
    ...
    for (size_t inputIndex = 0; inputIndex < inputsCount; inputIndex++) {
        const Isa::RegisterIndex registerIndex = getInputOutputRegisterIndex(true, inputIndex);
        setBit(usedRegisterMask, registerIndex);
    }

    // Clear registers which are not used
    for (Isa::RegisterIndex registerIndex = 0; registerIndex < Isa::generalPurposeRegistersCount; registerIndex++) {
...
Isa::RegisterIndex ShaderUnit::getInputOutputRegisterIndex(bool input, uint32_t index) const {
    switch (index) {
    case 0:
...
Isa::RegisterIndex ShaderUnit::getUniformRegisterIndex(uint32_t index) const {
    switch (index) {
    case 0:
